...
- The PDF's Topicsmaster row is the owner/control row for that shared block, not a second individual stopwatch. Its button controls the same aggregate clock shown in the dashboard, its real time mirrors that clock, and its status is labeled as linked.
